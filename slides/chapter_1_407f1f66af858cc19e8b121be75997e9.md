...
X = df[['age','balance','duration','marital']]
y = df['target']
...
The train and test dataset has 3,029 and 1,492 observations respectively.{{1}}
```
len(X_train)
len(X_test)
```{{1}}
...
`@script`
Before we build our model, we need to split our data to train and test datasets. The train data is used to develop a model and the test data is used to evaluate the performance of the model. This type of assessment is called Honest assessment. It is useful because it helps us understand the stability of model on an unseen data. While providing the split, we specify the size of the test dataset using the test_size option. Every time when we run the train_test split, the resulting dataset will differ since it uses a random number to split the data. If we need to replicate the same split results each time, we need to use the random_state option as shown here.
...
`@part1`
_**marital** _ - _single, married, divorced_.  

One of the below option can be used to convert categorical to continuous.
