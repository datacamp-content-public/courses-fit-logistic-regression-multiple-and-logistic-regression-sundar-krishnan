...
`@script`
In python, we can use two packages to train a logistic regression model. The first one is statsmodels package and second one is the widely used sklearn package. The input variables to both the packages will accept only continuous variables. For now, let us use the continuous variables in our data to build a logistic regression model.
...
`@script`
Before we build our model, we need to split our data to train and test datasets. After we develop a model using the training dataset, we can evaluate the performance of a model using the data which it has not seen before. In this case, it is the test dataset. This type of assessment is called Honest assessment. It is useful when we need to deploy a model in production.
